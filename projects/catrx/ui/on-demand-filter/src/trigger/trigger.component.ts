...
export class TriggerComponent implements OnInit, OnDestroy {
  ...
  @ViewChild('filterFormContent', { static: false })
  private elFilterFormContent?: ElementRef<HTMLDivElement>;
    ...
    this.filterOptionsControl.valueChanges
      ...
      .subscribe((value) => {
        this.filteredOptions = this.config.options.filter((option) =>
          this.getOptionConfig(option).fieldConfig.name.includes(value)
        );
      });
  }
    ...
    setTimeout(() => {
      ...
      optionConfig.fieldConfig.onChange = async (value) => {
        ...
        const selectedOptionIndex = selectedOptions.findIndex(
          (option) => option.name === optionConfig.fieldConfig.name
        );

        if (selectedOptionIndex >= 0 && !Array.isArray(value)) {
          selectedOptions[selectedOptionIndex] = {
            ...selectedOptions[selectedOptionIndex],
            previewValue: await this.getPreviewValue(
    ...
    switch (fieldConfig.type) {
      ...
      case 'autocomplete':
        options = await firstValueFrom(
          (fieldConfig as CatFormSelectOptions).options
        );
        this.closeDropdown();
  ...
  private autofillFilter() {
    if (this.config.autofill) {
      Object.keys(this.config.autofill).forEach((propName) => {
        const filterOption = this.config.options.find(
          (option) => this.getOptionConfig(option).fieldConfig.name === propName
        );
